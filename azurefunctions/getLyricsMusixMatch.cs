...
{
    public static class GetLyricsMusixMatch
    {
        [FunctionName("GetLyricsMusixMatch")]
            ...
            ILogger log)
        {
            ...
            string track_id = req.Query["track_id"];
            // Verificar si los parámetros son válidos
            if (string.IsNullOrEmpty(track_id) || string.IsNullOrEmpty(track_id))
            {
                return new BadRequestObjectResult("Please provide a track_id parameter.");
            }
            ...
            using (var client = new HttpClient())
            {
                ...
                string baseUrl = "https://api.musixmatch.com/ws/1.1/track.lyrics.get";
                string url = $"{baseUrl}?track_id={WebUtility.UrlEncode(track_id)}&apikey={apiKey}";
                try
                {
